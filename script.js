...
// escapa caracteres de HTML para impedir que dados do usuário (ex.: "luz & água",
// "lucro > meta", aspas) quebrem o markup injetado via innerHTML
function esc(s) {
  return String(s ?? '').replace(/[&<>"']/g, c =>
    ({ '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#39;' }[c]));
}

// id único; randomUUID evita colisão quando várias cópias são criadas no mesmo ms
function uid() {
...
// formata um campo de CSV: envolve em aspas, dobra aspas internas (RFC 4180) e
// neutraliza injeção de fórmula (=, +, -, @) ao abrir no Excel
function csvField(s) {
  let v = String(s ?? '');
  if (/^[=+\-@]/.test(v)) v = "'" + v;
  return '"' + v.replace(/"/g, '""') + '"';
...
// gera as cópias mensais pendentes das transações recorrentes
applyRecurrences();
...
// mês atual no formato 'YYYY-MM' (horário local, para não errar na virada do mês)
function nowYM() {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;
}

// soma um mês a um 'YYYY-MM'
function nextYM(ym) {
...
// data do mesmo dia do modelo dentro do mês alvo, ajustando meses mais curtos
function dateInMonth(srcDate, ym) {
  const day = Number(srcDate.slice(8,10));
  const [y,m] = ym.split('-').map(Number);
  const lastDay = new Date(y, m, 0).getDate();
  return `${ym}-${String(Math.min(day, lastDay)).padStart(2,'0')}`;
}

// cria as cópias mensais que faltam de cada transação recorrente, até o mês atual.
// Idempotente via marca-d'água (lastGenYM): não regenera meses já criados, então
// apagar uma cópia é permanente.
function applyRecurrences() {
  const hoje = nowYM();
  let changed = false;

  // só os "modelos": recorrentes que não são cópias geradas (sem parentId)
  transactions.filter(t => t.recorrente && !t.parentId).forEach(g => {
    let last = g.lastGenYM || g.data.slice(0,7);

    while (nextYM(last) <= hoje) {
      last = nextYM(last);
      transactions.push({
        id: uid(),
        parentId: g.id,
        tipo: g.tipo, desc: g.desc, valor: g.valor, cat: g.cat,
        data: dateInMonth(g.data, last),
        recorrente: true
      });
      changed = true;
    }

    if (g.lastGenYM !== last) { g.lastGenYM = last; changed = true; }
  });
...
function itemHTML(t) {
  ...
  <div class="transaction-item${t.recorrente?' recorrente-item':''}">
    <div class="transaction-info">
      ...
      <div>
        <div class="transaction-desc">${esc(t.desc)}${t.recorrente?'<span class="recorrente-tag">🔁 recorrente</span>':''}</div>
        <div class="transaction-meta">${esc(t.cat)} · ${fmtDate(t.data)}</div>
      </div>
    </div>
...
function renderMetas() {
  ...
  el.innerHTML = metas.map(m => {
    ...
    <div class="meta-card${concluida?' meta-concluida':''}">
      ...
      <div class="meta-top">
        <div>
          <div class="meta-nome">${concluida?'✅ ':''} ${esc(m.nome)}</div>
...
function renderInsights() {
  ...
  // filtra só os modelos recorrentes de despesa (cada um = um compromisso mensal)
  const recorrentes = transactions.filter(t=>t.recorrente && !t.parentId && t.tipo==='despesa');
...
function exportCSV() {
  ...
  const rows = transactions.map(t =>
    `${fmtDate(t.data)},${t.tipo==='receita'?'Receita':'Despesa'},${csvField(t.desc)},${csvField(t.cat)},${t.valor.toFixed(2)},${t.recorrente?'Sim':'Não'}`
  );
...
function exportPDF() {
  ...
  const rows = transactions.map(t =>
    `<tr style="border-bottom:1px solid #eee">
      ...
      <td style="padding:6px 10px">${esc(t.desc)}</td>
      <td style="padding:6px 10px">${esc(t.cat)}</td>
